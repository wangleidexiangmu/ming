 ...
 */
function catch_vogue_jetpack_setup() {
	/**
	...
	if ( 'infinite-scroll' === $pagination_type ) {
		add_theme_support( 'infinite-scroll', array(
			...
			'render'    => 'catch_vogue_infinite_scroll_render',
			'footer'    => 'page',
			'wrapper'   => false,
		) );
	}
...
add_action( 'after_setup_theme', 'catch_vogue_jetpack_setup' );

/**
 * Custom render function for Infinite Scroll.
 */
function catch_vogue_infinite_scroll_render() {
	while ( have_posts() ) {
		the_post();
		if ( is_search() ) :
