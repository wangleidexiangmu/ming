 ...
 */
function catch_vogue_customize_register( $wp_customize ) {
	...
	if ( isset( $wp_customize->selective_refresh ) ) {
		$wp_customize->selective_refresh->add_partial( 'blogname', array(
			...
			'render_callback' => 'catch_vogue_customize_partial_blogname',
		) );
		$wp_customize->selective_refresh->add_partial( 'blogdescription', array(
			'selector' => '.site-description',
			'container_inclusive' => false,
			'render_callback' => 'catch_vogue_customize_partial_blogdescription',
		) );
	}

	// Important Links.
	$wp_customize->add_section( 'catch_vogue_important_links', array(
		'priority'      => 999,
		'title'         => esc_html__( 'Important Links', 'catch-vogue' ),
	) );

	// Has dummy Sanitizaition function as it contains no value to be sanitized.
	catch_vogue_register_option( $wp_customize, array(
			'name'              => 'catch_vogue_important_links',
			'sanitize_callback' => 'sanitize_text_field',
			'custom_control'    => 'Catch_Vogue_Important_Links',
			'label'             => esc_html__( 'Important Links', 'catch-vogue' ),
			'section'           => 'catch_vogue_important_links',
			'type'              => 'catch_vogue_important_links',
		)
	);
	// Important Links End.
}
add_action( 'customize_register', 'catch_vogue_customize_register' );

/**
 * Binds JS handlers to make Theme Customizer preview reload changes asynchronously.
 */
function catch_vogue_customize_preview_js() {
	wp_enqueue_script( 'catch-vogue-customize-preview', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'assets/js/customizer.min.js', array( 'customize-preview' ), '20170816', true );
}
add_action( 'customize_preview_init', 'catch_vogue_customize_preview_js' );
